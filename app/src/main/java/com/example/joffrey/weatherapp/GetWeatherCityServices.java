...
public class GetWeatherCityServices extends IntentService {
    ...
    private void handleActionWeather() {
        ...
        try {
            url = new URL("http://api.openweathermap.org/data/2.5/forecast?APPID=b5fe018318b91102e114c2e5db8138d8&lat=" + Location.getCity());
            Log.i(TAG, "URL : " + url);
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("GET");
            conn.connect();
            if (HttpsURLConnection.HTTP_OK == conn.getResponseCode()) {
                copyInputStreamToFile(conn.getInputStream(), new File(getCacheDir(), "city_weather.json"));
